...
/*export checkObjectProperties(object, listRequirements){
  const keys = Object.keys(object),
  propertiesNotFound = [],
  propertiesFound = [];
  
  for(let i of listRequirements){
    if(keys.find(key => key === i))
     propertiesNotFound.push(i);
    else propertiesFound.push(i);
  }
  
  return {propertiesNotFound: propertiesNotFound,propertiesFound: propertiesFound, allRequirementsFulfilled: (propertiesNotFound.length < 1 && propertiesFound.length == listRequirements.length)};
} */